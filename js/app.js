...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const comfyFormElements = {
        ...
        max_prompt_weight: getById('comfy-max-prompt-weight'),
    ...
    async function saveSettings() {
        ...
        const settings = {
            ...
            max_prompt_weight: getFloatInputValue(comfyFormElements.max_prompt_weight, 1.2),
    ...
    async function loadSettings() {
        try {
            ...
            setInputValueIfDefined(comfyFormElements.max_prompt_weight, settings.max_prompt_weight);
    ...
    async function updateLoraListForModel(modelName) {
        ...
        const noFilter = noFilterCheckbox ? noFilterCheckbox.checked : false;

        try {
            const response = await fetchWithUserContext(`/api/comfyui/loras_by_model?model_name=${encodeURIComponent(modelName)}&no_filter=${noFilter}`);
            if (!response.ok) {
    ...
    async function fetchAndPopulateCheckpoints() {
        try {
            ...
            const checkpointsResponse = await fetchWithUserContext(`/api/comfyui/checkpoints?no_filter=${noFilter}`);
    ...
    async function selectModel(item) {
        ...
        if (isQwenModel) {
            ...
            // [v2.3] SDXL 模型自動設定
            if (newArchitecture === 'sdxl' || newArchitecture === 'pony') {
                // 1. 開啟「使用負面提示詞」
                const useNegativePromptCheckbox = document.getElementById('comfy-use-negative-prompt');
                if (useNegativePromptCheckbox) {
                    useNegativePromptCheckbox.checked = true;
    ...
    // =====================================================
    // [v18.0] 與 AI 商量功能 (已修正範圍與 fetching)
    // =====================================================
    {
            ...
            try {
                ...
                const response = await fetchWithUserContext('/api/comfyui/consult-ai', {
                    ...
                    body: JSON.stringify({
                        ...
                        maxPromptWeight: getFloatInputValue(comfyFormElements.max_prompt_weight, 1.2),
        ...
        if (modelNoFilterCheckbox) {
            modelNoFilterCheckbox.addEventListener('change', async () => {
                if (typeof fetchAndPopulateCheckpoints === 'function') {
                    await fetchAndPopulateCheckpoints();
                }
            });
        }
        ...
        if (loraNoFilterCheckbox) {
            loraNoFilterCheckbox.addEventListener('change', async () => {
                if (comfyFormElements && comfyFormElements.model && typeof updateLoraListForModel === 'function') {
                    await updateLoraListForModel(comfyFormElements.model);
                }
            });
        }
