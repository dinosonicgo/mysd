...
# v10.8 (AI 優化情境細化): 針對 AI 優化模式，為 'main' (核心) 和 'fixed' (固定) 兩種提示詞類型建立了完全獨立的 LLM 指令集。當優化核心提示詞時，新指令會嚴格禁止 LLM 添加任何通用的品質或風格標籤 (如 masterpiece, 8k)，強制其僅專注於擴展主題、場景和構圖細節。此修正從根本上解決了 LLM 擅自污染核心提示詞、導致固定提示詞前後置功能錯亂的問題。
...
**--- [v10.10 新增] 技術性指令：核心實體不變原則 (最高優先級) ---**
...
你的任務是：專注於將用戶的核心創意擴展為一個生動、詳細的場景。
**【嚴格規則】**
...
**【工作流程】**
1.  **分析核心創意**: 理解用戶想要描繪的主要內容，並鎖定核心實體名詞。
2.  **擴展視覺元素**: 圍繞核心創意，豐富場景細節、角色特徵、動態感和氛圍，但絕不替換核心實體。
3.  **建構提示詞**: 將擴展後的元素組合成一個連貫的、僅包含主題描述的提示詞，並為2-6個最相關的詞組分配不超過 1.4 的權重。
"""
            else:
